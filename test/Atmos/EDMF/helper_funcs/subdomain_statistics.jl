...
function compute_subdomain_statistics!(
    ...
    if (
        en.q_tot_cv > eps(FT) &&
        en.e_int_cv > eps(FT) &&
        fabs(en.e_int_q_tot_cv) > eps(FT) &&
        en.q_tot > eps(FT) &&
        sqrt(en.q_tot_cv) < en.q_tot
    )
        ...
        sd2_hq = log(
            corr * sqrt(en.q_tot_cv * en.e_int) / (en.e_int * en.q_tot) + FT(1),
        )
        ...
        for j_qt in 1:N_quad
            ...
            for j_I in 1:N_quad
                e_int_hat =
                    exp(μ_eint_star + sqrt2 * σ_cond_θ_q * abscissas[j_I])
                ...
                inner_src[i_SI_I] +=
                    eint_rain_src * e_int * weights[j_I] * sqpi_inv
                inner_src[i_Sqt_qt] += -qr_src * q_tot * weights[j_I] * sqpi_inv
                inner_src[i_SI_qt] +=
                    eint_rain_src * q_tot * weights[j_I] * sqpi_inv
            end
    ...
    en.cld_frac = t_cloudy = outer_env[i_T]
