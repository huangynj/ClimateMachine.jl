...
# CUDA.allowscalar(true) # for CPU tests
...
function mem_copy!(m::NTupleContainingModel{N}, dst::Vars, src::Vars) where {N}
    ...
    up = ntuple(i -> src.ntuple_model[i].scalar_model.x, N)

    # ntuple(N) do i # works for simple case, but not use-case
    unroll_map(N) do i
        dst.ntuple_model[i].scalar_model.x = up[i]
    end
end
...
@testset "ScalarModel" begin
    ...
    d_src = CuArray(a_src)
    d_dst = CuArray(a_dst)
    d_src .= CUDA.rand(FT, ns, n_elem)
    fill!(d_dst, 0)

    work_groups = (1,)
    ndrange = (n_elem,)
    device = CUDADevice()
    # device = CPU() # must use CuArray.allowscalar(true)
